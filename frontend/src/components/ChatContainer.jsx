...
const ChatContainer = () => {
  ...
  return selectedUser ? (
        ...
        <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
            {messages.map((msg, index) => (
                <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
                    {msg.image ? (
                        <img src={msg.image} alt="" className='max-w-[230px] rounded-lg border-gray-700 overflow-hidden mb-8' />
                    ) : (
    ...
    <div className='h-full flex flex-col justify-center items-center gap-2 bg-white/10 max-md:hidden'>
